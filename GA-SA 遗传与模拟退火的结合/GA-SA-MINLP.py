    ...
    def __newX(self, x0, D, isInt):
        ...
        for i in range(D):
            if isInt[i] == True:
                ...
                x[i] += floor((s * (2 * rand() - 1)))
        return x

    def compute(self, isInt):
        f = self.f
        niter = self.niter
        T, ft = self.T, self.fT
        ac = self.ac
        ...
        while T > ft:
            for i in range(niter):
                ...
                y, ret_Y = f(x), f(ret_X)
                new_X = self.__newX(x0=x, D=D, isInt=isInt)
                new_Y = f(new_X)
                dy1 = new_Y - y
                dy2 = new_Y - ret_Y
                if dy1 < 0:
...
class GSA:
    ...
    def compute(self, plot=True, bf=None):
        ...
        sa.set_x0(x0=x0)
        print("----- SA -----")
        ret_x, ret_y, tp2, yp2 = sa.compute(isInt=isInt)
        if plot == True:
...
if __name__ == '__main__':
    ...
    isInt = [True, True, True, True, False]
