...
class GM_11:
    ...
    def f_lsq(self, AB) -> np.ndarray:
        ''' 最小二乘目标 '''
        res = []
        for i in range(1, self.n):
            tmp = self.ax1[i] + AB[0] * self.x0[i] - AB[1]
            res.append(tmp)
        return array(res)
    ...
    def __white_eq(self, a, b, t):
        y1 = (b / (a ** 2) - self.x0[0] / a) * exp(-a * t)
        y2 = t * b / a + (1 + a) * self.x0[0] / a - b / (a ** 2)
        return y1 + y2
    ...
    def run(self):
        ...
        err = zeros(shape=self.x0.shape).astype(np.float) # 相对误差
        err1 = err.copy() # 残差
        for i in range(0, self.m):
            self.x1_p[i] = self.__white_eq(a, b, i)
...
# demo1
data = [2.878, 3.278, 3.39, 3.679, 3.77, 3.8]
